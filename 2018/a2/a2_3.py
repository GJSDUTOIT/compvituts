...
def driver():
    skull =  Image.open("ambassadors_skull.jpg")
    skull.show()
    skulldata = np.asarray(skull)
    warped_skull_data = projectify(skulldata,gen_H())
    warped_skull_image = Image.fromarray(warped_skull_data)
    warped_skull_image.save("warped_skull.jpg")
    warped_skull_image.show()
def p(a,b):
    return (-a * b)
...
def projectify(A,H):
    m, n, c = A.shape
    warped_skull_data = cv2.warpPerspective(A,H,(n,m))
    return warped_skull_data
