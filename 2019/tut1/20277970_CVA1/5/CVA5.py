...
def set_im1_name(name):
    global img1
    img1 = cv2.imread(name, cv2.IMREAD_COLOR)
    img1 = image_resize(img1, width=300)
def set_im2_name(name, i):
    global img2
    img2 = cv2.imread(name, cv2.IMREAD_COLOR)
    img2 = image_resize(img2, width=int(300*i))
def init():
    global orb
    orb = cv2.ORB_create()
...
def match_and_display_images(threshold=50):
    ...
    #cv2.imwrite("semper_match.jpg", matching_result)
    cv2.imshow("Matching result", matching_result)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
def example1():
    set_im1_name("pantheon.jpg")
    init()
    range_list = np.arange(1.0, 3.0, 0.1)
    for i in range_list:
        set_im2_name("pantheon.jpg",i)
